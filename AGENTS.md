...
./gradlew test                        # unit tests (Kotest, JUnit platform)
...
- `SkilletApp.kt` — Application class, Koin `appModule` initialization
- `MainActivity.kt` — `ComponentActivity` root, wraps in `KoinContext`, provides `CompositionLocal` for `NavController`/`NavigationActions`
...
- **`kotlin.code.style=official`** — use `kotlinfmt` or Android Studio formatter; avoid manual reformatting that breaks idiomatic style.
- **No CI, no pre-commit hooks** — manual verification only.
