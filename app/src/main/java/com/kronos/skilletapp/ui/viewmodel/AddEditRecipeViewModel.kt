...
class AddEditRecipeViewModel(
  ...
  fun updateName(name: String) {
    _recipeState.update { it.copy(name = name) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updateDescription(description: String) {
    _recipeState.update { it.copy(description = description) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updateNotes(notes: String) {
    _recipeState.update { it.copy(notes = notes) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updateServings(servings: Int) {
    _recipeState.update { it.copy(servings = servings) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updatePrepTime(prepTime: Int) {
    _recipeState.update { it.copy(prepTime = prepTime) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updateCookTime(cookTime: Int) {
    _recipeState.update { it.copy(cookTime = cookTime) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updateSource(source: String) {
    _recipeState.update { it.copy(source = source) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updateSourceName(sourceName: String) {
    _recipeState.update { it.copy(sourceName = sourceName) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun updateImage(image: String?) {
    _recipeState.update { it.copy(image = image) }
    _recipeState.checkForChanges(originalRecipeState)
  }
  ...
  fun moveIngredient(from: Int, to: Int) {
    _recipeState.update { it.copy(ingredients = it.ingredients.move(from, to)) }
    _recipeState.checkForChanges(originalRecipeState)
  }
  ...
  fun removeInstruction(instruction: Instruction) {
    _recipeState.update { it.copy(instructions = it.instructions - instruction) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun moveInstruction(from: Int, to: Int) {
    _recipeState.update { it.copy(instructions = it.instructions.move(from, to)) }
    _recipeState.checkForChanges(originalRecipeState)
  }
  ...
  fun removeEquipment(equipment: Equipment) {
    _recipeState.update { it.copy(equipment = it.equipment - equipment) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun moveEquipment(from: Int, to: Int) {
    _recipeState.update { it.copy(equipment = it.equipment.move(from, to)) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun showMessage(message: String) {
    _recipeState.update { it.copy(userMessage = message) }
    _recipeState.checkForChanges(originalRecipeState)
  }

  fun userMessageShown() {
    _recipeState.update { it.copy(userMessage = null) }
    _recipeState.checkForChanges(originalRecipeState)
  }
