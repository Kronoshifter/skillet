...
@Serializable data class SharedRecipe(val url: String, val id: String)
...
sealed interface Route {
  ...
  companion object {
    ...
    fun basePath(route: KClass<out Route>): String =
      "$BASE_URL/" +
        when (route) {
...
fun KClass<out Route>.toRouteString(): String {
  ...
  return buildString {
    ...
    properties
      .groupBy { it.returnType.isMarkedNullable }
      .let { grouped ->
...
class SkilletNavigationActions(private val navController: NavHostController) {
  ...
  fun navigateToRecipe(recipeId: String) {
    navController.navigate(Route.Recipe(recipeId)) { popUpTo<Route.RecipeList>() }
  }
