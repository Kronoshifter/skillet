...
class IngredientVisitor : IngredientGrammarBaseVisitor<Ingredient>() {
  ...
  override fun visitIngredient(ctx: IngredientGrammarParser.IngredientContext): Ingredient {
    ...
    val quantity =
      with(ctx.measurement()?.quantity()) {
        this?.decimal()?.text?.toFloatOrNull()
          ?: this?.fraction()?.let {
            when (it.NUMBER().size) {
              2 ->
                Fraction(
                  numerator = it.NUMBER(0).text.toInt(),
                  denominator = it.NUMBER(1).text.toInt(),
                )
              else ->
                Fraction(
                  whole = it.NUMBER(0).text.toInt(),
                  numerator = it.NUMBER(1).text.toInt(),
                  denominator = it.NUMBER(2).text.toInt(),
                )
            }.decimal
          }
      } ?: 0f
