...
inline fun <T> T.mutateIf(condition: Boolean, block: T.() -> T): T =
  if (condition) {
    block()
  } else {
    this
...
fun <A, B> Pair<A, B>.haveSameTypes(vararg klasses: KClass<*>): Boolean = klasses.any {
  it.isInstance(first) && it.isInstance(second)
}
